...
# 코랩 연결 코드
COLAB_URL = "https://1947-34-32-199-149.ngrok-free.app"  # 코랩(URL 넣기)
...
@csrf.exempt        #CSRF 제거(카카오 API에 한해서)
def kakao_request():
    data = request.get_json()
    news_title = data["action"]["params"]['sys.text']  # 카카오톡 메시지에서 뉴스 제목 추출

    tokenized_title_data = make_okt_token(news_title)
    predicted_label, predicted_percent = make_nlp_predict(tokenized_title_data)
    predicted_percent = predicted_percent  # 백분율로 변환
